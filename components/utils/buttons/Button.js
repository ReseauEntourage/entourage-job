...
const Button = (props) => {
    ...
    const type = {
        button: (
            <button {...attr} className={cssClassNames} type={props.type || 'button'} disabled={props.disabled}>
                {props.body}{props.children}
            </button>
        ),
        close: (
            <a {...attr} className={`uk-close ${cssClassNames}`} href='#' />
        ),
        link: (
            <a {...attr} className={cssClassNames} href='#' role='button'>
                {props.body}{props.children}
            </a>
        )
    };
