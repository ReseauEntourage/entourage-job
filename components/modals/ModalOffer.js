...
export const List = ({
  ...
  children
    ...
    <ul className={`uk-nav ${className}`}>
      {Array.isArray(children) ? children.map((item, i) => <li key={i}>{item}</li>) : children}
    </ul>
  );
List.propTypes = {
...
export const OfferInfoContainer = ({
  ...
  return <GridNoSSR gap="small" eachWidths={['auto', 'expand']}>
...
const ModalOffer = ({
  ...
  const updateOpportunityUser = async opportunityUser => {
    await axios.put(`${process.env.SERVER_URL}/api/v1/opportunity/join`, opportunityUser);
    setCurrentOffer({
      ...currentOffer,
      opportunityUser
    });
  ...
  useEffect(resetNoteBuffer, [currentOffer]); // futur: use moment

  const date = new Date(currentOffer.date);
  const formatDate = `${date.getMonth() + 1}/${date.getDate()}/${date.getFullYear()}`;
  return <div id="modal-offer" data-uk-modal="bg-close:false">
    <div className={`uk-modal-dialog uk-width-1-1 uk-width-3-4@m uk-width-2-3@l uk-width-1-2@xl ${archived && 'uk-light uk-background-secondary'}`}>
      <CloseButtonNoSSR onClick={() => {
        resetNoteBuffer();
        setClose(true);
      }} />
      {!currentOffer ? null : <div className="uk-modal-body">
        <GridNoSSR gap="small" between middle>
          ...
          <List className="uk-iconnav uk-grid-medium uk-flex-middle">
            ...
            <ButtonIcon name="pull" className={archived ? 'ent-color-amber' : undefined} onClick={() => {
              setLoadingIcon(true);
              const {
                userOpportunity
              } = currentOffer;
            ...
            <ButtonIcon name="star" className={bookmarked ? 'ent-color-amber' : undefined} onClick={() => {
              setLoadingIcon(true);
              const {
                userOpportunity
              } = currentOffer;
        ...
        <hr />
        <GridNoSSR className="uk-margin-bottom" eachWidths={['1-3@s', '2-3@s']} items={[<GridNoSSR column gap="medium">
          <GridNoSSR eachWidths={['expand', 'auto']} row middle>
            <Select id="modal-offer-status" title="Statut" name="status" placeholder="statut" options={OFFER_STATUS} value={status} onChange={event => {
        ...
        </GridNoSSR>, <GridNoSSR gap="medium">
        ...
        <div>
          <Textarea id="modal-offer-comment" name="modal-offer-comment" title="Ecrivez un commentaire à propos de cette opportunité..." type="text" value={noteBuffer} onChange={e => setNoteBuffer(e.target.value)} />
          {noteBuffer === note || note === null && noteBuffer === '' ? <Button style="default" disabled>
            Enregistré
              ...
              Enregistrer
                    {loading ? <div data-uk-spinner="ratio: 0.5" className="uk-margin-small-left" /> : null}
            </Button>}
        </div>
      </div>}
    </div>
